...
class Images extends CI_Controller {
	...
	public function index($file=null)
	{
		...
		// Get our params
		$assets	= $this->input->get('assets') ? $this->input->get('assets') : $this->config->item('upload_folder');
		$size	= $this->input->get('size');
		$height	= $this->input->get('height');
		$width	= $this->input->get('width');
		$crop	= $this->input->get('crop');
		$force	= $this->input->get('force');
		$ext = pathinfo(FCPATH.$this->config->item('upload_folder').$file, PATHINFO_EXTENSION);
		...
		if(!is_dir(FCPATH .$this->config->item('upload_folder'). "cache/")){
			mkdir(FCPATH .$this->config->item('upload_folder'). "cache/",0777);
		}

		$new_file = FCPATH .$this->config->item('upload_folder'). "cache/". str_replace('.'. $ext, '', $file) ."_{$width}x{$height}.". $ext;
